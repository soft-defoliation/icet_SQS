...
class ClusterSpaceConfig(BaseModel):
    ...
    cutoffs: list[float] = Field(
        default=Defaults.CUTOFFS,
        description="团簇截断半径列表 (Å)"
    )
    symprec: float = Field(
...
class SQSConfig(BaseModel):
    """SQS生成配置"""
    method: str = Field(
        default=MethodConfig.ENUMERATION,
        description="生成方法: enumeration 或 mc"
    )
    ...
    include_smaller_cells: bool = Field(
        default=Defaults.INCLUDE_SMALLER_CELLS,
        description="允许返回更小的优化结构"
    )
    
    # MC方法参数
    supercell_matrix: list[list[int]] = Field(
        default=Defaults.SUPERCELL_MATRIX,
        description="超胞扩胞矩阵"
    )
    
    # 通用参数
    pbc: tuple[bool, bool, bool] = Field(
        default=Defaults.PBC,
        description="周期边界条件"
    )
    ...
    early_stop_no_improve: int = Field(
        default=Defaults.EARLY_STOP_NO_IMPROVE,
        ge=1,
        description="连续无改进停止阈值"
    )
    ...
    T_start: float = Field(
        default=Defaults.T_START,
        gt=0,
        description="MC初始温度"
    )
    T_stop: float = Field(
        default=Defaults.T_STOP,
        gt=0,
        description="MC终止温度"
    )
    ...
    def supercell_multiplier(self) -> int:
        ...
        if self.supercell_matrix:
            return self.supercell_matrix[0][0] * self.supercell_matrix[1][1] * self.supercell_matrix[2][2]
        return 1
...
class OutputConfig(BaseModel):
    ...
    filename_prefix: str = Field(
        default=Defaults.FILENAME_PREFIX,
        description="文件名前缀"
    )
...
class ValidationConfig(BaseModel):
    """验证配置"""
    check_correlation: bool = Field(
        default=True,
        description="检查团簇向量相关性"
    )
    tolerance: float = Field(
        default=QualityThresholds.EXCELLENT,
        gt=0,
        description="验证容差"
    )
...
class SQSWorkflowConfig(BaseModel):
    ...
    # 可选：掺杂配置（替代dop.in）
    doping: Optional[dict[str, DopingSiteConfig]] = Field(
        default=None,
        description="掺杂配置（可选）"
    )
    ...
    def get_target_concentrations(self) -> dict[str, dict[str, float]]:
        ...
        return {
            name: site.concentration
            for name, site in self.doping.items()
        }
