...
class UniversalTemplateGenerator:
    ...
    def generate_labeled_poscar(self):
        ...
        with open(output_file, 'w') as f:
            ...
            for i, (sym, pos) in enumerate(zip(symbols, positions)):
                f.write(f"  {pos[0]:15.9f} {pos[1]:15.9f} {pos[2]:15.9f}   {sym}=1.0  # index {i}\n")
    ...
    def generate_index_guide(self):
        ...
        lines.append("=" * 80)
        lines.append(f"{'Index':<8} | {'Element':<8} | {'Status':<15} | {'Fractional coordinates':<30}")
        lines.append("-" * 80)
...
def main():
    ...
    if generator.load_structure():
        ...
        print(f"     (带标注的POSCAR模板)")
        print(f"  2. {guide_file}")
        print(f"     (原子索引对照表)")
        print(f"\n下一步:")
        print(f"  编辑模板文件，修改掺杂配置")
        print(f"  例如: 将 'K=1.0' 改为 'K=0.5,Na=0.5'")
        print(f"\n然后运行SQS工作流生成超胞")
    else:
        sys.exit(1)
