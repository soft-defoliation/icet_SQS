...
def run():
    ...
    lines = [
        f"SQS: {sqs_data['formula']} | {sqs_data['n_atoms']} atoms | "
    ...
    print("-" * 50)
    print(
        f"  {sqs_data['formula']} | {sqs_data['n_atoms']} 原子 | "
        f"{sqs_data.get('method','?')} | {sqs_data.get('generation_time',0):.1f}s"
    )
    print(f"  → {FileNames.FINAL_VASP} | output/ (中间文件) | " f"output/QUALITY_REPORT.txt (详细报告)")
    print()
