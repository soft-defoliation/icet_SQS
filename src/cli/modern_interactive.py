...
class ModernSQSInterface:
    ...
    def _first_time_setup(self):
        console.print(Panel.fit("[bold cyan]首次运行 — 需要生成掺杂配置模板[/bold cyan]", border_style="cyan"))
        ...
        with console.status("[cyan]生成模板...[/cyan]"):
            try:
                ...
                if generator.load_structure():
                    ...
                    console.print("\n[green]✓ 模板生成成功[/green]")
                    console.print("  [cyan]dop.in[/cyan]   — 编辑此文件设置掺杂浓度")
                    console.print("  [bold]sqskit[/bold] — 完成后重新运行")
    ...
    def _choose_method(self) -> str:
        ...
        return questionary.select(
            ...
            choices=[
                questionary.Choice(
                    title="[1] 枚举法 (Enumeration)", value="enumeration", description="全局最优解，结果尺寸灵活"
                ),
                questionary.Choice(
                ...
                questionary.Choice(title="[q] 退出", value="quit", description="退出程序"),
            ],
        ).ask()
    ...
    def _configure_quick(self, config: Dict) -> Dict:
        ...
        config["cluster_space"]["cutoffs"][0] = float(
            questionary.text(
                "团簇截断半径 (A):",
        ...
        if config["method"] == "enumeration":
            ...
            matrix_str = questionary.text(
                ...
                default="2 2 2",
                validate=lambda x: (
                    len(x.split()) == 3 and all(self._validate_int(n, 1, 10) for n in x.split())
                ),
            ).ask()
    ...
    def _configure_standard(self, config: Dict) -> Dict:
        ...
        if config["method"] == "montecarlo":
            ...
            config["sqs"]["early_stop_no_improve"] = int(
                questionary.text(
                    "无改进停止次数:",
            ...
            config["sqs"]["include_smaller_cells"] = questionary.confirm(
                "允许返回更小的优化结构?", default=config["sqs"]["include_smaller_cells"]
            ).ask()
    ...
    def _configure_clusterspace_expert(self, config: Dict) -> Dict:
        ...
        cutoffs_str = questionary.text(
            ...
            validate=lambda x: (
                all(self._validate_float(v.strip(), 1.0, 20.0) for v in x.split(",") if v.strip())
            ),
        ).ask()
        config["cluster_space"]["cutoffs"] = [
            float(v.strip()) for v in cutoffs_str.split(",") if v.strip()
        ]

        config["cluster_space"]["symprec"] = float(
            questionary.text(
                "对称性容差 (symprec):",
        ...
        config["cluster_space"]["position_tolerance"] = float(
            questionary.text(
    ...
    def _configure_enum_expert(self, config: Dict) -> Dict:
        ...
        config["sqs"]["random_seed"] = (
            int(config["sqs"]["random_seed"]) if config["sqs"]["random_seed"] else None
        )

        pbc_str = questionary.text("周期边界条件 (3个True/False，空格分隔):", default="True True True").ask()
    ...
    def _configure_mc_expert(self, config: Dict) -> Dict:
        ...
        config["sqs"]["save_progress"] = questionary.confirm(
            "保存优化进度?", default=config["sqs"]["save_progress"]
        ).ask()
    ...
    def _configure_output_expert(self, config: Dict) -> Dict:
        ...
        config["output"]["directory"] = questionary.text(
            "输出目录:", default=config["output"]["directory"]
        ).ask()

        config["output"]["filename_prefix"] = questionary.text(
            "文件名前缀:", default=config["output"]["filename_prefix"]
        ).ask()

        config["output"]["save_intermediate"] = questionary.confirm(
            "保存中间文件?", default=config["output"]["save_intermediate"]
        ).ask()
    ...
    def _configure_validation_expert(self, config: Dict) -> Dict:
        ...
        config["validation"]["check_correlation"] = questionary.confirm(
            "检查团簇向量相关性?", default=config["validation"]["check_correlation"]
        ).ask()

        config["validation"]["tolerance"] = float(
            questionary.text(
                "验证容差:",
    ...
    def _print_summary(self):
        console.print()
        table = Table(
            box=box.ROUNDED, title="[bold green]✓ 生成完成[/bold green]", title_style="bold green"
        )
