    ...
    for iteration in range(max_iter):
        try:
            ...
            deviation = calculate_cv_deviation(
                cs, sqs, target_concentrations=target_conc, use_icet_comparison=False
            )
            quality, passed, _ = evaluate_sqs_quality(
                deviation, len(supercell), n_disordered, target_conc
            )
...
def run():
    ...
    print(
        f"  超胞 {n_mult} ({len(original_structure)}→{len(supercell)} 原子) | "
    ...
    sqs, deviation, n_iter = generate_sqs_iterative(
        cs=cs,
        supercell=supercell,
        target_conc=target_concentrations,
        tolerance=tolerance,
        max_iter=max_iter,
        progress_file=(
            Path(FileNames.PROGRESS_PKL) if sqs_config.get("save_progress", True) else None
        ),
    ...
    # 修正晶格
    expected_lattice = np.array(original_lattice) * np.diag(
        [supercell_matrix[i][i] for i in range(3)]
    )
    sqs = Atoms(
        symbols=sqs.get_chemical_symbols(),
        scaled_positions=sqs.get_scaled_positions(),
        cell=expected_lattice,
        pbc=True,
    )

    print(
        f"  → {sqs.get_chemical_formula()} | {len(sqs)} 原子 | "
    ...
    sorted_sqs = Atoms(
        symbols=[a[0] for a in sorted_atoms],
