...
from src.utils.quality_utils import calculate_cv_deviation, evaluate_sqs_quality
from src.parser import StructureParser
from src.constants import Tolerances, FileNames


def validate_target_concentrations(
    target_concentrations: dict[str, dict[str, float]],
    tolerance: float = Tolerances.CONCENTRATION
) -> bool:
    """
    验证目标浓度配置。
    ...
    for site_label, conc_dict in target_concentrations.items():
        ...
        if abs(total - 1.0) > tolerance:
            raise ValueError(
                f"位点 '{site_label}' 的浓度总和为 {total:.6f}，必须等于 1.0"
            )
        
        for elem, conc in conc_dict.items():
            if not 0 <= conc <= 1:
                raise ValueError(
                    f"位点 '{site_label}' 中元素 '{elem}' 的浓度 {conc} 超出 [0, 1] 范围"
                )
...
def generate_sqs_on_fixed_supercell(
    ...
    if n_sqs != n_target:
        ...
        print(f"  建议: 调整 max_size 或 supercell_matrix")
    ...
    if len(occupations) != len(user_disordered_indices):
        print(f"\n  ✗ 错误: occupation 数量 ({len(occupations)}) ≠ 用户无序位点数 ({len(user_disordered_indices)})")
        print(f"  原因: 原胞与常规晶胞的无序位点比例不匹配")
        print(f"  建议: 检查 dop.in 配置，确保与 ClusterSpace 一致")
        raise ValueError(f"Occupation 数量不匹配: {len(occupations)} != {len(user_disordered_indices)}")
...
def save_sqs_results(
    ...
    # 按元素排序的 VASP 格式
    sorted_atoms = sorted(
        zip(sqs.get_chemical_symbols(), sqs.positions),
        key=lambda x: x[0]
    )
