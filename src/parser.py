...
class StructureParser:
    ...
    def find_dop_in(cwd: Optional[Path] = None) -> Path:
        ...
        raise FileNotFoundError(
            f"未找到dop.in文件。查找位置: {FileNames.DOP_IN}, {FileNames.DOP_IN_ALT}"
        )
    ...
    def _parse_coordinates(self) -> list[DopingSite]:
        ...
        for line_idx, line in enumerate(self._lines[8:], start=0):
            ...
            if label:
                ...
                raise ValueError(
                    f"第{line_idx+9}行缺少元素标注！"
                    f"格式: x y z Element=1.0 # index N"
                )
    ...
    def _build_target_concentrations(
        self,
        sites: list[DopingSite]
    ...
    def parse_for_supercell(self, filepath: Optional[Path] = None) -> tuple[Atoms, np.ndarray, float]:
        """
        简化的解析，用于超胞构建
